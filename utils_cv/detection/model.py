...
from src.my_utils import get_all_files_in_folder
...
class DetectionLearner:
    ...
    def get_last_exp(self, folder):
        checkpoints = get_all_files_in_folder(Path(folder), ['*.pth'])
        exps = [0]
        for c in checkpoints:
            # checkpoint_exp_0_e0_precision_0.2818_recall_0.4072.pth
            exps.append(int(c.stem.split('_')[2]))
    ...
    def fit(
        ...
        if step_size is None:
            step_size = int(np.round(epochs / 6))  # 1.5
        ...
        max_exp = self.get_last_exp('logs')
        ...
        for epoch in range(self.epochs):
            ...
            if not skip_evaluation:
                ...
                # save checkpoint
                stats = e.coco_eval['keypoints'].stats
                if stats[0] > best_precision or stats[5] > best_recall:
                    best_precision = stats[0]
                    best_recall = stats[5]
                    best_epoch = epoch
                    ...
                    torch.save(
                        checkpoint,
                        os.path.join('logs',
                                     'checkpoint_exp_{}_e{}_precision_{}_recall_{}.pth'.format(max_exp, epoch,
                                                                                               round(stats[0], 3),
                                                                                               round(stats[5], 3))))
